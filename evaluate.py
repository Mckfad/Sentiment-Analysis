...
Comparaison avec un modèle baseline (TextBlob / règles simples)
"""
...
def evaluate_model(n_samples: int = 200, output_dir: str = "."):
    ...
    print(classification_report(y_true, y_distilbert, target_names=["NEGATIVE", "POSITIVE"]))
    ...
    # ── Graphiques
    _plot_confusion_matrices(y_true, y_distilbert, y_baseline, output_dir)
...
def _plot_f1_comparison(report_bert, report_base, output_dir):
    ...
    fig.suptitle("Comparaison DistilBERT vs Baseline", fontsize=14, fontweight="bold")

    for ax, bert_v, base_v, cls in zip(axes, bert_vals, base_vals, classes):
        b1 = ax.bar(x - width/2, bert_v, width, label="DistilBERT", color="#4C9BE8")
...
if __name__ == "__main__":
    ...
    print(f"\n🎯 Accuracy DistilBERT : {results['accuracy_distilbert']:.2%}")
    print(f"🎯 Accuracy Baseline   : {results['accuracy_baseline']:.2%}")
    print(f"🎯 F1 DistilBERT       : {results['f1_distilbert']:.4f}")
